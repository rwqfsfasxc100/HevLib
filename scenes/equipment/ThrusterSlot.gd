				...
				if not valid_scene:
					...
					match aux_type:
						...
						"THRUSTER","RCS","TORCH","MAIN_PROPULSION":
							...
							item.maxMissalignment = data.get("max_misalignment",0.349)
							...
							if flare:
								flare.set_deferred("essentiality",data.get("flare_essentiality",0.5))
							...
							for i in data.get("extra_nozzles",[]):
								nozzles.append(convert_to_nozzle(i))
							...
							if nozzleA:
								...
								nozzleA.rotation = deg2rad(nd.rotation)
								var rs = nd.scale
								if rs.size() >= 2:
							...
							for n in nozzles:
								...
								if thisNozzle:
									thisNozzle.coolTime = nd.cool_time
									thisNozzle.heatTime = nd.heat_time
									var t = "res://ships/modules/nozzle-cd.png"
									if file.file_exists(nd.texture):
										t = nd.texture
									var tr = load(t)
									thisNozzle.texture = tr
									var l = "res://ships/modules/nozzle-n.png"
									if file.file_exists(nd.normal):
										l = nd.normal
									var tl = load(l)
									thisNozzle.normal_map = tl
									var h = "res://ships/modules/nozzle-cl.png"
									if file.file_exists(nd.heat):
										h = nd.heat
									var th = load(h)
									var heat = thisNozzle.get_node_or_null("heat")
									if heat:
										heat.texture = th
									thisNozzle.region_enabled = nd.region_enabled
									var rr = nd.region_rect
									if rr.size() >= 4:
										thisNozzle.region_rect = Rect2(rr[0],rr[1],rr[2],rr[3])
									thisNozzle.region_filter_clip = nd.region_filter_clip
									var rp = nd.position
									if rp.size() >= 2:
										thisNozzle.position = Vector2(rp[0],rp[1])
									thisNozzle.rotation = deg2rad(nd.rotation)
									var rs = nd.scale
									if rs.size() >= 2:
										thisNozzle.scale = Vector2(rs[0],rp[1])
